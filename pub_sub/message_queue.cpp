...
//   						cout << "deliver " << cur->get_type() << " from " << cur->get_publisher() << " topic  " << cur->get_topic() << " Buffer size: " << it->first.size() << endl;
            while (cur != 0)
            {
                ...
                std::string* cur_pub = 0;
								if(alist != 0)
								{
									for ( list<MessageBuffer*>::iterator buf_it = alist->begin(); buf_it != alist->end(); buf_it++)
									{
											cur_pub = new std::string();
											if(cur->has_sec_publisher())
											{
												cur_pub->assign (cur->get_sec_publisher());
												cout << "has sec pub " << endl;
											}
											else
											{
												cur_pub->assign(cur->get_publisher());
												cout << "publisher " << endl;
											}

											if ((*buf_it)->getOwner() == (*cur_pub) );
											else
											{
